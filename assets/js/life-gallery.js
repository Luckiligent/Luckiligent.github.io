(() => {
  ...
  const render = () => { const item = activeImages[index]; image.src = item.src; image.alt = item.alt || albumTitle; title.textContent = item.caption || albumTitle; count.textContent = `${index + 1} / ${activeImages.length}`; };
  const move = (delta) => { index = (index + delta + activeImages.length) % activeImages.length; render(); };
  document.querySelectorAll('[data-life-album]').forEach((card) => card.addEventListener('click', () => { activeImages = JSON.parse(card.dataset.lifeAlbum); if (!activeImages.length) return; trigger = card; albumTitle = card.dataset.lifeTitle; index = 0; render(); dialog.showModal(); }));
